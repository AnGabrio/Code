...
#saveRDS(cea_res,"cea_res_univ_all_submission.rds")
cea_res<-readRDS("cea_res_univ_all_submission.rds")
icer<-cea_res$ICER
...
#bootstrapping utilities
boot_lmm_uc <- function(data, i){
  data2 <- data[i,]
  data_long_boot<-reshape(data2[i,], varying = c("eq5d_1","eq5d_2","eq5d_3","cost_1","cost_2","cost_3"),
                          direction = "long",idvar = "id",sep = "_", new.row.names=sequence(657))
  data_long_boot$time_f2<-ifelse(data_long_boot$time==2,1,0)
  data_long_boot$time_f3<-ifelse(data_long_boot$time==3,1,0)
  data_long_boot$trt_time_f2<-data_long_boot$trt*data_long_boot$time_f2
  ...
  return(c(summary(em3_u_ml.eq5d)$emmean[c(1,2,3,5,9)], 
           ...
           summary(em3_u_ml.eq5d.delta)$estimate, 
           summary(cgm3_u_ml.qalys.delta)$estimate,
           summary(em3_c_ml.cost)$emmean[c(1,2,3,5,9)], 
           summary(em3_c_ml.cost)$emmean[5]- summary(em3_c_ml.cost)$emmean[2],
           summary(em3_c_ml.cost)$emmean[9]- summary(em3_c_ml.cost)$emmean[3],
           summary(em3_c_ml.cost.delta)$estimate, 
           summary(cgm3_c_ml.cc.tcosts)$estimate))
...
set.seed(4567) #v1
boot_est_lmm_uc <- boot(data_wide, boot_lmm_uc, R=10000)
delta_e_boot<-boot_est_lmm_uc$t[,10]
mu_e1_boot<-boot_est_lmm_uc$t[,8]
mu_e2_boot<-boot_est_lmm_uc$t[,9]
mu_e_boot<-cbind(mu_e1_boot,mu_e2_boot)
delta_c_boot<-boot_est_lmm_uc$t[,20]
mu_tc1_boot<-boot_est_lmm_uc$t[,18]
mu_tc2_boot<-boot_est_lmm_uc$t[,19]
mu_tc_boot<-cbind(mu_tc1_boot,mu_tc2_boot)


#cep and ceac
library(BCEA)
cea_res2<-bcea(e=mu_e_boot,c=mu_tc_boot, ref=2, Kmax = 40000)
#saveRDS(cea_res2,"cea_res_univ_all_submission.rds")
cea_res2<-readRDS("cea_res_univ_all_submission.rds")
icer<-cea_res2$ICER
paste_icer<-paste("ICER",round(icer,0),sep = "=")

opar <- par(ps=18) 
plot(cea_res2$delta.e,cea_res2$delta.c,xlim = c(-0.1,0.12),ylim = c(-10000,10000),main ="Cost-Effectiveness Plane",
     xlab = "Expected QALY differential",ylab = "Expected cost differential",type="n")
points(cea_res2$delta.e,cea_res2$delta.c,col="#7cb9b9",pch=16,cex=.7)
points(mean(cea_res2$delta.e),mean(cea_res2$delta.c),col="#007C7C",pch=16,cex=1)
abline(h=0,lty="dashed")
abline(v=0,lty="dashed")
abline(a=0,b=25000)
text(-0.04,-5000,"k=25000",cex = 0.7)
...
#CEAC
opar <- par(ps=18) 
plot(cea_res2$k,cea_res2$ceac,xlim = c(0,40000),ylim = c(0,1),main ="Cost-Effectiveness Acceptability Curve",
     xlab = "Acceptance threshold",ylab = "Probability of Cost-Effectiveness",type="n",axes=F)
axis(1)
axis(2)
lines(cea_res2$k,cea_res2$ceac,col="#007C7C",lty=1,lwd=3)
